...
class ZI_HDAWG8(ZI_base_instrument):
    ...
    def calibrate_dio_protocol_old(self):
        # TODO: add docstrings to make it more clear
        print("Checking that all bits of DIO protocol toggle...")
        done = 4*[False]
        timeout = 5
        while not all(done):
            ok = True
    ...
    def configure_codeword_protocol(self, default_dio_timing: bool=False):
        ...
        for awg_nr in range(4):
            ...
            # N.B. DIO bit 8 (first of 2nd byte)  not connected in AWG8!
                ...
                if 1: # FIXME: this is from HEAD on TUD203148
                    self.set('awgs_{}_dio_mask_value'.format(awg_nr), 2**6-1)
                    # self.set('awgs_{}_dio_mask_value'.format(awg_nr), 2**3-1)
                    # Figuring out the shift is a little bit hairy. For the moment,
                    # I simply hard code it. The reason being that for V0 hardware
                    # bit 8 wasn't working, so the block with that bit is skipped
                    # in the CC-Lite.
                    if awg_nr == 0:
                        mask_shift = 0
                    elif awg_nr == 1:
                    ...
                    self.set('awgs_{}_dio_mask_shift'.format(awg_nr), mask_shift)
                    # self.set('awgs_{}_dio_mask_shift'.format(awg_nr), 3)
                else: # this is from branch master on Github
                    # self.set('awgs_{}_dio_mask_value'.format(awg_nr), 2**6-1)
    ...
    def plot_dio_data(self, bits):
        data = self._dev.getv('raw/dios/0/data')
        ZI_HDAWG8.plot_timing_diagram(data, bits, 48)

    def plot_awg_dio_data(self, awg, bits):
        data = self._dev.getv('awgs/' + str(awg) + '/dio/data')
        ZI_HDAWG8.plot_timing_diagram(data, bits, 48)

    def check_dio_timing_error(self, awgs):
        for awg in awgs:
            timing_error = self._dev.geti('awgs/' + str(awg) + '/dio/error/timing')
            if timing_error != 0:
