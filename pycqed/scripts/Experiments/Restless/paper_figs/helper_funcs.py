...
def extract_mn_sig_from_data_dict(data_dict):
    ...
    for i, n_cl in enumerate(n_cls):
        for j, att in enumerate(atts):
            ...
            std_eps[i, j] = np.mean(np.std(data_dict[n_cl][att],
                                           axis=1, ddof=1))
            # Standard error of mean on the 10 means and 10 std's measured
            sem_eps[i, j] = scipy.stats.sem(np.mean(data_dict[n_cl][att],
                                                    axis=1))
            sem_std[i, j] = scipy.stats.sem(np.std(data_dict[n_cl][att],
                                                   axis=1, ddof=1))
